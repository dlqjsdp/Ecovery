<!--
 * 무료나눔 게시판 상세보기 페이지
 * 사용자가 선택한 무료나눔 게시글의 상세 내용을 조회하는 페이지
 *
 * [🖼 프론트엔드 기능 - by eunji]
 * - 이미지 영역 구성 (대표 이미지, 썸네일 리스트)
 * - 카드 UI 구성: 제목, 배지, 작성자 정보, 상태 정보 등
 * - 댓글 작성 폼, 댓글 정렬 셀렉트 박스, 댓글 리스트 렌더링 구조 제공
 * - 목록으로 돌아가기 버튼 및 상단 빵부스러기 내비게이션 구성
 * - 비로그인 사용자에 대한 댓글 작성 제한 안내 메시지
 * - 레이아웃1 템플릿과 연동되며 `free-sharing-detail.js`와 연결됨

 * [🛠 백엔드 연동 기능 - by yeonsu]
 * - 게시글 상세 정보 객체 전달: `${board}`
 *   - 제목, 상태, 지역, 카테고리, 작성자 닉네임 등 Thymeleaf로 바인딩
 * - 로그인 사용자 정보 전달: `${loginMemberNickname}`, `${loginMember?.memberId}`
 * - 로그인 사용자 = 게시글 작성자 or ADMIN 권한이면 수정/삭제 버튼 노출
 * - 댓글 작성 가능 여부(로그인 시만 가능), 댓글 페이징용 `id="pagination"` 영역 제공
 * - JS에서 게시글 ID, 유저 정보 등을 활용하기 위한 `window.loginUserId`, `window.loginUserRole` 설정
 * - 동적 데이터는 JS(`free-sharing-detail.js`)에서 Ajax로 불러와 렌더링
 *
 * @author   : eunji, yeonsu
 * @fileName : free-sharing-detail.html
 * @since    : 250729
-->
