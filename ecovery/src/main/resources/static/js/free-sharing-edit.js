...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeForm() {
        ...
        showNotification('게시물 수정 페이지가 로드되었습니다.', 'info');
    }
    ...
    function validateForm() {
        ...
        if (!status) {
            showNotification('나눔 상태를 선택해주세요.', 'error');
    ...
    function submitForm() {
        ...
        .then(data => {
            if (data.success) {
                ...
                // 2초 후 상세 페이지로 이동
                setTimeout(() => {
                    window.location.href = `/free-sharing/detail/${window.itemData.id}`;
                }, 2000);
            } else {
    ...
    function deletePost() {
        ...
        // 삭제 API 호출
        fetch(`/free-sharing/delete/${window.itemData.id}`, {
            method: 'DELETE',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                // 1.5초 후 목록 페이지로 이동
                setTimeout(() => {
                    window.location.href = '/free-sharing';
                }, 1500);
            } else {
    ...
    // 이전 페이지로 이동
    function goBack() {
        if (document.referrer && document.referrer.includes('/free-sharing/detail/')) {
            window.history.back();
        } else {
            window.location.href = '/free-sharing';
        }
    ...
    const autoSave = debounce(function() {
        if (hasChanges) {
            ...
            // 로컬 스토리지에 임시 저장
            localStorage.setItem(`freesharing_draft_${window.itemData.id}`, JSON.stringify(formData));
    ...
    function restoreDraft() {
        const draftKey = `freesharing_draft_${window.itemData.id}`;
    ...
    console.log('✅ 무료나눔 수정/삭제 페이지 JavaScript 로드 완료');
});